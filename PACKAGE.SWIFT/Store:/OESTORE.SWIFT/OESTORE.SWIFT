...
public final class OEStore: ObservableObject {
    ...
    public func updateOfficialStatus(id: String, status: String) async {
        ...
        if let updated: Official = try? await api.update(entity: "RefUser", id: id, body: Patch(status: status)) {
    ...
    public func updateGameStatus(id: String, status: String) async {
        struct Patch: Codable { let status: String }
        if let updated: OEGame = try? await api.update(entity: "OEGame", id: id, body: Patch(status: status)) {
    ...
    public func assign(game: OEGame, official: Official) async {
        let a = OEAssignment(
            ...
            game_date: game.date, official_id: official.id,
    ...
    public func updateAssignmentStatus(id: String, status: String) async {
        struct Patch: Codable { let status: String }
        if let updated: OEAssignment = try? await api.update(entity: "OEAssignment", id: id, body: Patch(status: status)) {
